...
//
// Role-aware reports (mirrors the backend gates in routes/reports.ts):
//   - ADMIN: the four analytics tabs (Overview / Bookings / Payments /
//     Tours), each owning its own filter bar and per-card query states.
//   - AGENT: "My activity" (/reports/agent-activity — self-scoped).
//   - CUSTOMER: "My travel summary" (/reports/me).
"use client";
...
import { MyActivitySection } from "@/components/reports/MyActivitySection";
import { MyTravelSummarySection } from "@/components/reports/MyTravelSummarySection";
import {
  REPORTS_VIEW_COPY,
  reportsViewForRole,
...
const ReportsPage = () => {
  ...
  const view = reportsViewForRole(roleOf(user));
  const copy = REPORTS_VIEW_COPY[view];

  return (
    <div
      className={`min-h-screen bg-background ${
        view === "admin-tabs" ? "pb-16 md:pb-0" : ""
      }`}
    >
      <div className="mx-auto w-full max-w-7xl">
        ...
        {view === "customer-summary" && <MyTravelSummarySection />}
        {view === "agent-activity" && <MyActivitySection />}
      ...
      {/* Phone tab bar — admin tabs only (mirrors the landing page's bottom nav) */}
      {view === "admin-tabs" && (
        <nav
