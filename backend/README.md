...
* **Money** is integer minor units (pesewas) in every column and payload — Paystack amounts compare without conversion.
...
* **Concurrency**: inventory is consumed *inside* transactions with guarded atomic `updateMany` (tour slots, flight seats) or a room-row lock + recount, so concurrent bookings can't oversell. Payment settlement is one transactional, idempotent step — duplicate webhooks can't re-confirm or re-send receipts.
...
* **Notifications** (`src/notifications/`) ride a durable BullMQ queue — 3 attempts, exponential backoff, exhausted jobs stay marked failed. `NOTIFICATIONS_INLINE=true` sends directly (tests, Redis-less local runs).
