...
* **Payments** settle by three independent paths: the signed `charge.success` webhook, the payer's return-trip verify, and a reconciliation sweep (`src/services/payment/reconcile.service.ts`, every 15 minutes) that asks Paystack about charges still pending after ten minutes. Refunds flip the ledger first under a guarded claim (so two concurrent refund clicks can never call Paystack twice), cancel the booking and release its inventory in the same transaction, then call the Paystack refund API; a rejected call releases the claim for retry, and a claim Paystack never received is re-issued by the sweep. `refund.processed` webhooks and a windowed refund-list sweep record refunds made on the Paystack dashboard, so the ledger never overstates revenue. All refunds are for the full charge; a partial provider-side refund is reported for manual handling.
